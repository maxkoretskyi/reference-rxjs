...
export function stream(name, delayOrValues, countOrValues, log?) {
    ...
    if (Array.isArray(countOrValues)) {
        if (Array.isArray(delayOrValues)) {
            console.assert(countOrValues.length === delayOrValues.length, 'Number of intervals and values should match');

            s = from(countOrValues).pipe(
                concatMap((v, index) => timer(delayOrValues[index]).pipe(mapTo(name + `-${v}`)))
            );
        } else {
            s = from(countOrValues).pipe(
        ...
        if (Array.isArray(delayOrValues)) {
            console.assert(countOrValues === delayOrValues.length, 'Number of intervals and values should match');
            ...
            s = timer(0, delayOrValues).pipe(map((v) => name + `-${v}`)).pipe(take(countOrValues));
        }
    }
